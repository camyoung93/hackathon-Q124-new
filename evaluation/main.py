...
def evaluate_model(start_time: datetime, stop_time: datetime):
    phoenix_client = px.Client(use_active_session_if_available=True)
    ...
    hallucination_df, qa_df = run_evals(
        qa_df, [hallucination_eval, qa_eval], provide_explanation=True
    )
...
if __name__ == "__main__":
    while True:
        ...
        start_time = (current_time - timedelta(minutes=2, seconds=15)).isoformat()
